...
class CinemaLogic {
    ...
    public function displayHalls($bioscoopId) {
        ...
        while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
            ...
            $html .= '</ul>';
            $html .= '<a href="./?op=reservatie&zaalId='.$row['zaal_id'].'&bioscoopId='.$bioscoopId.'&prijs=11" class="btn-link dark">Reserveer binnenkort <i class="fas fa-arrow-circle-right"></i></a>';
            $html .= '</div>';
            $html .= '</div>';
        }
